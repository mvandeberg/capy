...
// overloads can be exercised without a real io_context.
...
// Minimal io_context-shaped context for exercising on_context(): work runs
// inline through the executor, so run()/restart() are no-ops. Stands in for
// a real corosio::io_context, which this library cannot depend on.
struct immediate_context : test_io_context
{
    void run() noexcept {}
...
public:
    ...
    testOnContext()
    {
        // on_context() builds a conforming runner from any context with
        // get_executor()/run()/restart(). immediate_context runs work
        // inline, standing in for a real corosio::io_context.
