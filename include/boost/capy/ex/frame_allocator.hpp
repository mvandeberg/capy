...
/** Return the current frame allocator for this thread.
    ...
    The start functions (`run_async`, `run`) set the
    thread-local value before they invoke a child
    coroutine. The child's `promise_type::operator new`
    reads it to allocate the coroutine frame from the
    correct resource.
