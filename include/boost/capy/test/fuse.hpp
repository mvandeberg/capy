...
public:
    ...
    */
    ...
    /** Build a runner that drives each iteration on `ctx`.
        ...
        runner overload of @ref armed. It launches the task on `ctx` via
        `run_async`, pumps `ctx` to completion, readies it for the next
        round, and returns any exception the task raised (null on
        success). Because the exception is returned rather than rethrown,
        it cannot escape a `run_async` completion handler (which would
        call `std::terminate`); @ref armed rethrows it from fuse's own
        synchronous code.
        ...
        must outlive the @ref armed call.
