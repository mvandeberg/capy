...
task<void> parent()
{
    co_await child();  // child sees our environment
}
...
`this_coro::environment` never suspends—it returns immediately.
...
struct stoppable_timer
{
    ...
    void await_suspend(std::coroutine_handle<> h, io_env const* env)
    {
        ...
        auto handle = start_timer(duration_, [h, ex = env_->executor]() {
            ex.dispatch(h).resume();
        });

        std::stop_callback cb(env_->stop_token, [handle]() {
            cancel_timer(handle);
        });
    }
