...
Launch functions (`run_async`, `run`) own the `io_env` that is passed to
every awaitable in the chain. The `io_env` is guaranteed to outlive the
task and all awaitables it contains. This is a responsibility of the
launch function:

* `run_async` stores `io_env` in its trampoline coroutine frame, which
  outlives the launched task.
* `run` stores `io_env` in its awaitable, which persists for the entire
  `co_await` expression.

Because of this guarantee, awaitables that receive `io_env const*` in
`await_suspend` should store it directly; copying is unnecessary and wasteful.

[source,cpp]
----
std::coroutine_handle<> await_suspend(std::coroutine_handle<> h, io_env const* env)
{
    env_ = env;  // Correct: store pointer
    // ...
}
----
