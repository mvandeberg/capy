...
Tasks implement `IoRunnable`, receiving execution environment when
awaited:
...
task<void> child()
{
    ...
    // env.executor - the executor
    // env.stop_token - the stop token
}

task<void> parent()  // Bound to pool's executor
{
    co_await child();  // child inherits our environment
}
