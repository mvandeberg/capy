...
Capy supports _cooperative_ cancellation through {cpp}20's `std::stop_token`:
...
{cpp}20 provides three related types:
...
task<void> child()
{
    // Inherits parent's stop token via environment
    auto token = co_await this_coro::stop_token;
    if (token.stop_requested())
        co_return;
}
...
task<void> parent()
{
    // Our environment is passed to child
    co_await child();
}
...
`this_coro::environment` never suspends—it's intercepted by `await_transform` and
returns immediately.
...
struct stoppable_timer
{
    ...
    void await_suspend(std::coroutine_handle<> h, io_env const* env)
    {
        ...
        {
            cancelled_ = true;
            env_->executor.dispatch(h).resume();
            return;
        }

        // Start timer
        handle_ = start_timer(duration_, [h, ex = env_->executor]() {
            ex.dispatch(h).resume();
        });

        // Cancel timer if stop requested
        if (env_->stop_token.stop_possible())
        {
            // Note: stop_callback must outlive the operation
            // In real code, store in coroutine frame or awaiter
            std::stop_callback cb(env_->stop_token, [this]() {
...
| Query current environment (including stop token) inside coroutine
