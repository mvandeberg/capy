...
At completion, the task's `final_suspend` performs unconditional symmetric
transfer to its continuation:
...
Within a coroutine chain, every task shares the same executor, so the
continuation is always valid for direct transfer. When executors diverge
(via `run(other_ex)`), the `run` awaitable interposes a dispatch trampoline
that handles the executor switch on completion.

== Querying the Current Executor

Inside a coroutine, retrieve the current executor via the environment:
...
`this_coro::environment` is a tag that `await_transform` intercepts. It never
...
| Query current environment (including executor) inside coroutine
|===
