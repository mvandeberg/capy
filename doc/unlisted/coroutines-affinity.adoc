...
Within a coroutine, use `co_await this_coro::environment` to retrieve the current
execution environment that this coroutine is bound to:
...
The `this_coro::environment` is a tag that the promise intercepts via
...
| Retrieve the current environment (including executor) inside a coroutine
