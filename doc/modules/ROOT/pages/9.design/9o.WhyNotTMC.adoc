...
* Child tasks can run on different executors via cpp:run[]`(other_ex)(child_task())`
...
*Capy's design choice:* Intentionally prevent mid-coroutine executor switching. A coroutine stays on its bound executor for its entire lifetime. Child tasks can run on different executors via cpp:run[]`(other_ex)(child_task())`, but the parent never moves.
...
* Custom allocator propagation via cpp:run_async[]`(ex, allocator)` and cpp:run[]`(allocator)`
...
[source,cpp]
----
include::example$snippets/9o_why_not_tmc.cpp[tag=arena,indent=0]
----
...
[source,cpp]
----
include::example$snippets/9o_why_not_tmc.cpp[tags=std_suspend_void;std_suspend_bool;std_suspend_handle,indent=0]
----
...
[source,cpp]
----
include::example$snippets/9o_why_not_tmc.cpp[tag=await_suspend_env,indent=0]
----
...
** cpp:io_env::executor[env->executor] — The executor (where to resume)
** cpp:io_env::stop_token[env->stop_token] — A stop token (for cancellation)
** cpp:io_env::frame_allocator[env->frame_allocator] — An optional `std::pmr::memory_resource*` for coroutine frame allocation (null selects the default allocator)
...
[source,cpp]
----
include::example$snippets/9o_why_not_tmc.cpp[tag=strict_static_assert,indent=0]
----
...
* cpp:any_stream[], cpp:any_read_stream[], cpp:any_write_stream[]
* Write a function taking `any_stream&` - it compiles once
* Up to five vtable calls per I/O operation (four when synchronous), not one per continuation
...
* Native backends: epoll and io_uring (Linux), kqueue (BSD/macOS), IOCP (Windows), select (portable fallback)
...
| Threading
| Multi-threaded (cpp:thread_pool[])
...
| Serialization
| cpp:strand[] (ordering preserved across suspend)
...
| Stream concepts
| Yes (cpp:ReadStream[], cpp:WriteStream[], etc.)
| No
