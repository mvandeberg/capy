...
Capy type-erases the stream itself. This is possible because coroutines provide structural type erasure — the continuation is always a handle, not a template parameter. When the library is coroutines-only, five vtable calls per I/O operation is the total cost — not one per continuation. The completion handler, executor, and allocator do not need individual erasure because they are not part of the stream's operation signature.
