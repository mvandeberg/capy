...
This example reads from a cpp:ReadStream[] and writes to a cpp:WriteStream[] using `write_now`. Each chunk is drained to completion before the caller can read more from the source.
...
| cpp:WriteStream::write_some[write_some]
