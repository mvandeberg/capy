...
Equivalently, `n == buffer_size(buffers)` implies `!ec`: a fully satisfied transfer is always reported as success, with any condition (including EOF) deferred to the next call.
...
The `read_some` contract permits `n > 0` when `ec` is set. Data and errors are not mutually exclusive: the implementation reports exactly what happened. The one boundary is that a completely filled buffer (`n == buffer_size(buffers)`) is always reported as success, with any accompanying condition deferred to the next call. This is the most consequential design decision in the `ReadStream` concept, with implications for every consumer of `read_some` in the library. This section explains the design and its consequences.
...
The `ReadStream` concept permits both behaviors. Streams that naturally produce `(ec, 0)` on error (such as POSIX socket wrappers) conform. Streams that report `(ec, n)` with `n > 0` (such as TLS or compression layers) also conform. The one constraint is that a completely filled buffer is reported as success: if a transfer delivers exactly `buffer_size(buffers)` bytes, any accompanying condition is deferred to the next call rather than reported alongside the full count.
...
The same postcondition holds at every layer. Both `read_some` and the composed `read` report a completed transfer as success and attach a condition only to a partial transfer: on error, `n < buffer_size(buffers)`. A `read` that accumulates data across several internal `read_some` calls and then encounters EOF returns `(eof, m)` with `m < buffer_size(buffers)` -- the bytes that landed, plus the reason the fill stopped short. The shape of the result -- partial progress alongside an error code, and a full transfer reported as success -- is identical from the `read_some` primitive through the composed `read`.
