...
This document describes the design of the cpp:ReadStream[] concept: the fundamental partial-read primitive in the concept hierarchy. It explains why `read_some` is the correct building block and how composed algorithms build on top of it.
...
Once `read_some` returns an error (including EOF), a subsequent call is still permitted. A conforming stream may report the same condition again, report a different one, or resume delivering data -- the concept does not pick one of the three, so callers cannot assume the stream is exhausted for good.
