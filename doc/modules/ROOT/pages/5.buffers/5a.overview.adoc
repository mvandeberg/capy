...
* A `string_view` wrapped with cpp:make_buffer[] (which yields a single cpp:const_buffer[])
...
Capy provides cpp:const_buffer[] and cpp:mutable_buffer[] as semantically neutral buffer types, without imposing `std::byte` semantics. Their layout matches POSIX `iovec` directly; Windows `WSABUF` differs, so Capy copies descriptors there instead.
