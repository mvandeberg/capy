...
`std::string` and `std::string_view` are ranges of characters,
not of buffers, so they do not satisfy the concepts themselves. Wrap them
with cpp:make_buffer[] to obtain a single-buffer sequence.
...
2. The slice views produced by cpp:buffer_slice[] and cpp:consuming_buffers::data[]`()` need to adjust the first and last buffers' bounds
