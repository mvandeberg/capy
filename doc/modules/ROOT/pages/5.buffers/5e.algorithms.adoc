...
`buffer_size` returns the sum of bytes, not the count of buffers.
...
The algorithm fills target buffers sequentially and reads from source buffers as needed. It handles cases where a single source buffer spans multiple target buffers, or vice versa.
...
Data never moves unnecessarily. The buffer sequence points to existing data. The OS reads directly from those locations:
