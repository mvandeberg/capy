...
This section is the bridge between theory and practice. It shows how Capy turns C++20 coroutines into a complete async programming model. The topics run from starting and scheduling tasks, through cancellation and concurrent composition, to fine-grained control over memory allocation. Each topic builds on the last, and by the end you are writing real asynchronous programs with Capy.
