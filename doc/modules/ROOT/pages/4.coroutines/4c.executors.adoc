...
An *executor* is an object that can schedule work for execution. Beyond the two scheduling operations (`dispatch` and `post`), the concept also requires work tracking, equality comparison, access to the owning context, and nothrow copy/move:
...
concept Executor =
    ...
    requires(E const& ce, E const& ce2, continuation c) {
...
=== Work tracking and equality

`on_work_started()` and `on_work_finished()` maintain the context's outstanding-work count. Calls must be paired, and the context uses the count to decide when a graceful shutdown is complete. Ordinary user code does not call these directly—work guards invoke them. Two executors compare equal (`==`, `noexcept`) when they schedule onto the same context, and every executor is nothrow copy- and move-constructible.
