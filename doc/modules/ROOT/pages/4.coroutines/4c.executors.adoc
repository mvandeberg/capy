...
cpp:executor_ref[] stores a reference to the underlying executor—the original executor must outlive the `executor_ref`.
...
Work posted to a cpp:thread_pool[] may execute on any of its worker threads. If your coroutines access shared data, you must use appropriate synchronization.
...
Custom execution contexts inherit from cpp:execution_context[]:
