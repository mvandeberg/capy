...
The solution is to pass values as **function parameters** instead of **lambda captures**. The compiler copies function parameters to the coroutine frame.
...
This is an **Immediately Invoked Function Expression (IIFE)**. The compiler copies the parameter `s` to the coroutine frame before the first suspension, so it remains valid for the coroutine's lifetime.

== Complete Example

=== Broken: Lambda Captures
...
This pattern is rare. Most async code immediately invokes the lambda and discards it, which makes captures unsafe.
...
Proposals to change this behavior exist, but as of {cpp}23 the issue remains.
