...
`when_all` starts multiple cpp:io_task[] children concurrently and waits for all of them to complete. It returns cpp:task[task]`<io_result<R1, R2, ..., Rn>>`, a single `ec` plus the flattened payloads:
...
In the snippet above nothing dangles: `items` outlives the `when_all`. The
...
You have now learned how to compose tasks concurrently with `when_all` and `when_any`. The next section covers frame allocators for customizing coroutine memory allocation.
