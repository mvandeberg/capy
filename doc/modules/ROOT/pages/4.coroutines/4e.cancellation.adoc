...
wakes; the loser's wait resolves with cpp:cond::canceled[cond::canceled]. See
...
Connect UI cancellation to stop tokens. Pass the token through cpp:run_async[] so it propagates automatically via the execution environment. The task accesses it with `co_await this_coro::stop_token` instead of receiving it as a function argument:
...
cpp:when_any[] uses stop tokens internally to cancel "losing" tasks when the first task completes. This is covered in xref:4.coroutines/4f.composition.adoc[Concurrent Composition].
