...
Lazy execution enables efficient composition. Tasks that are never awaited never run and consume no resources beyond their initial allocation.
...
Without symmetric transfer, each `co_await` would add a stack frame. Deep nesting could then cause stack overflow. With symmetric transfer, `c` returning to `b` returning to `a` uses constant stack space regardless of depth.

Capy implements this through an `await_suspend` that returns a coroutine handle rather than `void`:
...
The task captures exceptions thrown inside it and rethrows them when awaited:
...
The promise stores the exception when it occurs. `await_resume` rethrows it when the calling coroutine resumes.
...
You have now learned how to declare, return values from, and await `task<T>` coroutines. In the next section, you will learn how to start tasks using `run_async` and `run`.
