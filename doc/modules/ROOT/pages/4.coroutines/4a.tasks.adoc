...
The syntax cpp:task[task<>] is equivalent to `task<void>` and represents a coroutine that completes without producing a value.
...
A critical property of cpp:task[task<T>] is *lazy execution*: creating a task does not start its execution. The coroutine body runs only when the task is awaited.
...
The cpp:task[task<T>] type is defined in:
