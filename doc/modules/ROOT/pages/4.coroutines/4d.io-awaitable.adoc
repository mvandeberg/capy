...
For tasks that can be started from non-coroutine contexts, the cpp:IoRunnable[] concept refines `IoAwaitable` and requires a `promise_type` plus the following:
...
These methods exist because launcher functions like `run_async` cannot `co_await` the task directly. The trampoline must be allocated before the task type is known, so it type-erases the task through function pointers. The trampoline therefore needs a common API to manage lifetime and extract results.

The context injection methods `set_continuation` and `set_environment` are part of the `IoRunnable` concept: it requires them on the `promise_type`. Launcher functions access them through the typed handle provided by `handle()`.
...
The escape hatch is to wrap the foreign awaitable (or callback, or future) in a small `IoAwaitable`. That awaitable captures the executor and re-posts the resumption through it:
...
You have now learned how the IoAwaitable protocol enables context propagation through coroutine chains. The next section covers stop tokens and cooperative cancellation.
