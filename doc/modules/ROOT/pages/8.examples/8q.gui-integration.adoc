...
The stand-in exists to prove the contract, not to model a toolkit. Four pieces change when the toolkit is real, and the rest does not:
...
* `dialog` becomes the toolkit's own dialog, and `show` the call that opens it. The callback becomes the toolkit's completion notification.
...
2. Make `show_dialog` cancellable. Inspect `env->stop_token` and complete early when a stop is already requested
