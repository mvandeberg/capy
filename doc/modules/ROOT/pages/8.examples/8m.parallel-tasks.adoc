...
cpp:when_all[] starts all four tasks concurrently on the thread pool. Each task may run on a different thread. The result is cpp:io_result[io_result]`<long long, long long, long long, long long>`, a single `ec` plus the four partial sums in input order.
