...
target_link_libraries(echo_lib PUBLIC Boost::capy)
...
The header declares only the signature. It includes cpp:any_stream[] and cpp:task[], but no concrete transport types.
