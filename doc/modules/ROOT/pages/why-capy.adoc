...
The following sections demonstrate this claim. Each section examines a domain where Capy innovates—not by reinventing what works, but by solving problems that have remained unsolved.
...
Asio's stream concepts are hybrid by design. Capy's are coroutine-only, which enables the optimizations above.
...
*Stop tokens propagate automatically.* Cancel at the top of your coroutine tree. Every nested operation receives the signal. Capy integrates with OS-level cancellation—`CancelIoEx` on Windows, `IORING_OP_ASYNC_CANCEL` on Linux. Pending I/O operations cancel immediately.
...
* `run`, `run_async` — functions that start a task, with forward-flow allocator control
