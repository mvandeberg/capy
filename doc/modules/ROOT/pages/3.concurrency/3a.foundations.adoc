...
Modern computers have multiple processor cores. A quad-core laptop can do four things at once. But most programs use only one core. The others sit idle. Concurrency lets you use all your processing power.
...
The benefits compound in computationally intensive work. Image processing, scientific simulations, video encoding—you can split these tasks into independent pieces. Process them simultaneously and your program finishes in a fraction of the time.
...
The `std::thread` constructor takes a function (or any *callable*). It immediately starts a new thread that runs that function. Two bookmarks now move through your code simultaneously.
...
The interleaving varies each run. Both threads race to print. Their outputs jumble together. This unpredictability is your first glimpse of concurrent programming's fundamental challenge: when threads share resources (here, `std::cout`), the output can become garbled.
...
Without `std::ref()`, the thread would modify a copy. `counter` would stay unchanged.
...
Every thread must be either *joined* or *detached* before its `std::thread` object is destroyed. Failing to do so calls `std::terminate()`. This abruptly ends your program.
...
Sometimes you want a thread to run independently. It continues even after the `std::thread` object is destroyed. That is what `detach()` does:
