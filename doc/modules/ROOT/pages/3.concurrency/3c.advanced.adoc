...
You can make any trivially copyable type atomic.
...
== Condition Variables: Efficient Waits

Sometimes a thread must wait for a specific condition before proceeding. You could loop and check repeatedly:
...
A condition variable lets one thread signal others that something has changed. Waiting threads sleep until notified. They consume no CPU.
