...
== Futures and Promises: Get Results Back
...
* You can call a future's `get()` only once
...
* `set_exception()` lets the worker signal an error
...
`std::async` starts the function (potentially in a new thread) and returns a future. No explicit thread creation, no promise management, no join call.
...
The code divides the work into chunks. Each thread handles one chunk. For CPU-bound work on large datasets, this can dramatically reduce execution time.
...
* *std::async* — A simpler way to start parallel work
