...
Between any of these steps, the other thread might execute its own steps. Imagine both threads read `counter` when it is 5. Both add one to get 6. Both write 6 back. Two increments, but the counter only went up by one. This is a *lost update*, a classic race condition.

The more threads, the more opportunity for races. The faster your processor, the more instructions execute between context switches. This can hide the bug—until one critical day in production.
...
But there is a problem with calling `lock()` and `unlock()` directly. If code between them throws an exception, `unlock()` never executes. The mutex stays locked forever. Any thread waiting for it blocks eternally—a *deadlock*.
...
The `std::lock_guard` locks the mutex on construction and unlocks it on destruction. Even if an exception is thrown, the destructor runs and releases the mutex. This is the correct way to use mutexes.

=== std::scoped_lock ({cpp}17)

Since {cpp}17, prefer `std::scoped_lock`. It works like `lock_guard`, but it can lock multiple mutexes at once. This avoids a class of deadlock:
...
For more control, use `std::unique_lock`. You can unlock it before destruction, move it to another scope, or create it without immediately locking it:
...
`std::scoped_lock` uses a deadlock-avoidance algorithm internally. It acquires both mutexes without risk of circular waiting.
