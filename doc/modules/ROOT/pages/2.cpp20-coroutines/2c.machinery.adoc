...
Every coroutine has an associated *promise type*. This type acts as a controller for the coroutine. It defines how the coroutine behaves at key points in its lifecycle. The promise type is not something you pass to the coroutine. It is a nested type inside the coroutine's return type that the compiler uses automatically.
...
* `final_suspend()` determines whether the coroutine frame persists after completion. If it returns `suspend_always`, you must manually destroy the coroutine. If it returns `suspend_never`, the runtime destroys the frame automatically

=== Trace Promise Behavior
...
== Put It Together: Build a Generator
...
The compiler transforms the expression `co_yield value` into:
