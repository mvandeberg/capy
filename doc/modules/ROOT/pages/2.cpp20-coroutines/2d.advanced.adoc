...
The key insight: returning a handle enables the compiler to implement the resumption as a tail call. The compiler reuses the current stack frame for the next coroutine.
...
`void`::
Always suspend. The coroutine suspends. Some external mechanism must resume it.
...
=== Use Symmetric Transfer in Generators
...
1. An implicit try-catch surrounding the coroutine body catches the exception
...
Exceptions thrown before the first suspension point (before `initial_suspend` completes) propagate directly to the caller without going through `unhandled_exception()`. If `initial_suspend()` returns `suspend_always`, the coroutine suspends before any user code runs. This avoids the edge case.

== Build a Production Generator
