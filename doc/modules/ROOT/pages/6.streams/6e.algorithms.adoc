...
task<> process_pipeline(any_buffer_source& source, any_buffer_sink& sink)
{
    ...
    for (;;)
    {
        ...
        for (const_buffer src : src_bufs)
        {
            auto processed = transform(src);

            // Write processed data into the sink's own storage
            auto dst_bufs = sink.prepare(dst_arr);
            std::size_t copied = buffer_copy(dst_bufs, make_buffer(processed));
