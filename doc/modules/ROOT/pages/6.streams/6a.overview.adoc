...
* Completed xref:../5.buffers/5f.dynamic.adoc[Buffer Sequences]
...
// bufs point into the source's internal data; call source.consume(n) when done

// BufferSink: callee provides writable buffers
mutable_buffer dst_arr[16];
auto wbufs = sink.prepare(dst_arr);
// Write into wbufs, then commit the number of bytes written
