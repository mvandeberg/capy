...
* Understanding of the three stream concepts
...
[source,cpp]
----
include::example$snippets/6b_streams.cpp[tag=read_some_signature,indent=0]
----
...
Equivalently, `n == buffer_size(buffers)` implies `!ec`. A read that fills the buffer sequence is a success even when the underlying operation also signals a condition such as end-of-stream. That condition is reported on a subsequent read.
...
[source,cpp]
----
include::example$snippets/6b_streams.cpp[tag=write_some_signature,indent=0]
----
...
Equivalently, `n == buffer_size(buffers)` implies `!ec`: a write that drains the entire buffer sequence is a success even when the underlying operation also signals a condition. That condition is reported on a subsequent write.
