...
A type satisfies cpp:ReadStream[] if it provides partial read operations via `read_some`:
...
A type satisfies cpp:WriteStream[] if it provides partial write operations via `write_some`:
...
Wraps any cpp:ReadStream[] in a type-erased container:
...
Wraps bidirectional streams (both cpp:ReadStream[] and cpp:WriteStream[]):
