...
Capy is not header-only, so link against `Boost::capy`:

[source,cmake,role=pseudocode]
----
add_executable(hello_coro hello_coro.cpp)
target_link_libraries(hello_coro PRIVATE Boost::capy)
----

`Boost::capy` carries its own thread-linkage requirements, so a plain
`-lcapy` command line cannot reproduce them. Build with `CMake`, then run
the executable:
