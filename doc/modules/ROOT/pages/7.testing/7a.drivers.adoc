...
| Like `armed(fn)` for coroutine lambdas, but drives each iteration
  ...
  task runs on the caller's execution context (for example an
  ...
  as a `std::exception_ptr` (null on success); `armed` rethrows it. Use
  `on_context` to build a conforming `run_one`.

| `on_context(ctx) -> run_one`
| Build a `run_one` that drives each iteration on `ctx` -- any type with
  `get_executor()`, `run()`, and `restart()` (e.g. `corosio::io_context`).
  Static; returns a `(IoRunnable) -> std::exception_ptr` callable.
...
`armed(run_one, fn)` overload. The caller supplies `run_one`, which runs each
iteration's task on that context; `fuse::on_context` builds one for any
context exposing `get_executor()`, `run()`, and `restart()`:
...
A hand-rolled `run_one` must run the task to completion and *return* any
exception the task raises as a `std::exception_ptr` (null on success);
`armed` rethrows it. Returning the exception -- rather than rethrowing it
from inside `run_async`'s completion handler, which would call
`std::terminate` -- is exactly what `on_context` does, so prefer it unless
you have an unusual drive loop.
