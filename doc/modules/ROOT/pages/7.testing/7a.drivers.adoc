...
| Like `armed(fn)` for coroutine lambdas, but drives each iteration
  through the caller-supplied `run_one` instead of `run_blocking`. The
  task therefore runs on any execution context the caller chooses, for
  example an `io_context`, which operations built on `corosio::timeout`
  require. `run_one` runs the task to completion and returns any escaped
  exception as a `std::exception_ptr` (null on success); `armed` rethrows it.
