...
The mock streams and buffer adapters complete synchronously, so they
check the token up front. An outstanding stop request yields cpp:error::canceled[error::canceled]
...
cpp:set_current_thread_name[] names the calling thread so that debuggers,
...
cpp:set_current_thread_name[] lives in namespace `boost::capy`, not
...
| cpp:set_current_thread_name[]`(char const* name)`
