...
| Limit bytes returned per `read_some` call on this end, simulating
  chunked network delivery. The limit applies to this end only; the peer
  end has its own independent limit.

| `provide(std::string_view sv)`
| Inject bytes into the peer's incoming buffer, bypassing the fuse.
  Resumes a suspended `read_some` on the peer if any.
