...
returns cpp:error::test_failure[error::test_failure].
...
Both `stream` ends satisfy cpp:ReadStream[] and cpp:WriteStream[].
...
`read_some` calls on the peer return cpp:cond::eof[cond::eof]. The peer may still
...
cpp:cond::eof[cond::eof]. All subsequent operations on both ends return
`cond::eof`.
...
terminated by `'\n'` from a cpp:ReadStream[]. The `fuse.armed()` loop
