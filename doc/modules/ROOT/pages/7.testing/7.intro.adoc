...
non-deterministic, and do not fail on demand. As a result, error-handling
paths go untested until production breaks them. Capy ships a self-contained
toolkit that replaces the transport with in-memory mocks, drives coroutines
to completion on the calling thread, and injects failures at every
`maybe_fail()` site. This exercises every error branch automatically.
...
* xref:7.testing/7a.drivers.adoc[Drive Tests] -- `run_blocking` drives a
  ...
  `maybe_fail()` site in turn until it covers every failure path. The
  `thread_name` header's `set_current_thread_name` function labels
  worker threads, so failures in multi-threaded tests are easier to
  attribute.
...
in exception mode. Meanwhile, `run_blocking` keeps the whole thing on the
...
Continue to xref:7.testing/7a.drivers.adoc[Drive Tests] to begin.
