...
Every coroutine has an associated *promise type*. This type acts as a controller for the coroutine, defining how it behaves at key points in its lifecycle. The promise type is not something you pass to the coroutine.
...
`get_return_object()`::
Called to create the object returned to the caller of the coroutine.
...
[source,cpp]
----
include::example$programs/Bc_machinery_trace.cpp[tag=full]
----
...
The promise is constructed first, then `get_return_object()` creates the return value, then `initial_suspend()` runs. Since `initial_suspend()` returns `suspend_never`, the coroutine body executes immediately. Since `final_suspend()` returns `suspend_always`, the coroutine suspends one last time, and the promise is not destroyed until the coroutine handle is explicitly destroyed.
...
[source,cpp]
----
include::example$snippets/Bc_machinery.cpp[tag=from_promise,indent=0]
----
...
[source,cpp]
----
include::example$programs/Bc_machinery_generator.cpp[tag=full]
----
...
[source,cpp]
----
include::example$programs/Bc_machinery_fibonacci.cpp[tag=full]
----
...
The Fibonacci generator runs an infinite loop internally. But because it yields and suspends after each value, the caller controls when (and whether) to ask for more values.
