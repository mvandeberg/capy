...
NOTE: Capy has no futures. This page is background, not API. It covers the
standard-library result-passing model that Capy leaves behind, so
xref:why-capy.adoc[Why Capy?] has something concrete to argue against. Nothing on
this page appears in Capy's own interface.

== Futures and Promises: Getting Results Back

Passing references works but is clunky. {cpp} offers a cleaner abstraction: *futures* and *promises*.
...
[source,cpp]
----
include::example$programs/Cd_patterns_promise_future.cpp[tag=full]
----

The main thread calls `get()`, which blocks until the value is available.
...
[source,cpp]
----
include::example$programs/Cd_patterns_async.cpp[tag=full]
----
...
[source,cpp]
----
include::example$snippets/Cd_patterns.cpp[tag=launch_policies,indent=0]
----
...
[source,cpp]
----
include::example$programs/Cd_patterns_thread_local.cpp[tag=full]
----

Each thread sees its own `counter`. No synchronization needed because the data is not shared.
...
[source,cpp]
----
include::example$snippets/Cd_patterns.cpp[tag=thread_safe_queue,indent=0]
----
...
[source,cpp]
----
include::example$programs/Cd_patterns_producer_consumer.cpp[tag=full]
----
...
[source,cpp]
----
include::example$programs/Cd_patterns_parallel_for.cpp[tag=full]
----

For CPU-bound work on large datasets, this can dramatically reduce execution time.
...
Capy's concurrency facilities—cpp:thread_pool[], cpp:strand[], cpp:when_all[], cpp:async_event[], cpp:async_mutex[], and cpp:async_waker[]—build on these standard primitives for coroutine-friendly concurrent programming.
