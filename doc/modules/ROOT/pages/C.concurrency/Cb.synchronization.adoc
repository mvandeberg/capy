...
[source,cpp]
----
include::example$programs/Cb_synchronization_race.cpp[tag=full]
----

Two threads, each incrementing 100,000 times. You would expect 200,000. But run this repeatedly and you see different results—180,000, 195,327, maybe occasionally 200,000.

The `++counter` operation looks atomic—indivisible—but it is not. It consists of three steps:
...
The faster your processor, the more instructions execute between context switches, potentially hiding the bug—until one critical day in production.

== Mutual Exclusion: Mutexes

The solution to data races is *mutual exclusion*: ensuring that only one thread accesses shared data at a time.

A *mutex* (mutual exclusion object) is a lockable resource. Before accessing shared data, a thread *locks* the mutex. If another thread already holds the lock, the requesting thread blocks until the lock is released.

[source,cpp]
----
include::example$programs/Cb_synchronization_mutex.cpp[tag=full]
----
...
{cpp} has a powerful idiom: *RAII* (Resource Acquisition Is Initialization). The idea: acquire resources in a constructor, release them in the destructor. Since destructors run even when exceptions are thrown, cleanup is guaranteed.

Lock guards apply RAII to mutexes:

[source,cpp]
----
include::example$snippets/Cb_synchronization.cpp[tag=lock_guard]
----

The `std::lock_guard` locks the mutex on construction and unlocks it on destruction. Even if an exception is thrown, the destructor runs and the mutex is released.

=== std::scoped_lock ({cpp}17)

Since {cpp}17, `std::scoped_lock` is preferred. It works like `lock_guard` but can lock multiple mutexes simultaneously:

[source,cpp]
----
include::example$snippets/Cb_synchronization.cpp[tag=scoped_lock,indent=0]
----
...
[source,cpp]
----
include::example$snippets/Cb_synchronization.cpp[tag=unique_lock,indent=0]
----

`std::unique_lock` is more flexible but slightly more expensive than `std::lock_guard`.
...
[source,cpp]
----
include::example$snippets/Cb_synchronization.cpp[tag=deadlock]
----
...
[source,cpp]
----
include::example$snippets/Cb_synchronization.cpp[tag=scoped_lock_multi]
----
