...
[source,cpp]
----
include::example$programs/Ca_foundations_hello.cpp[tag=full]
----

The `std::thread` constructor takes a function (or any *callable*) and immediately starts a new thread running that function.

The `join()` call makes the main thread wait until thread `t` finishes. Without it, `main()` might return and terminate the program before `say_hello()` completes.

=== Parallel Execution

[source,cpp]
----
include::example$programs/Ca_foundations_parallel.cpp[tag=full]
----
...
The interleaving varies each run: both threads race to print, and when they share a resource like `std::cout`, output can jumble together.
...
[source,cpp]
----
include::example$programs/Ca_foundations_lambda.cpp[tag=full]
----

The lambda captures `x` by value—it copies `x` into the lambda. By default, `std::thread` also decays and copies every argument passed to its constructor, before invoking the callable.

To pass by reference, use `std::ref()`:

[source,cpp]
----
include::example$programs/Ca_foundations_ref.cpp[tag=full]
----

Without `std::ref()`, this fails to compile. `std::thread` decays arguments to values, and a decayed `int` cannot bind to `increment`'s `int&` parameter.
...
[source,cpp]
----
include::example$programs/Ca_foundations_member.cpp[tag=full]
----
...
[source,cpp]
----
include::example$snippets/Ca_foundations.cpp[tag=join,indent=0]
----

=== detach()

`detach()` lets a thread run independently, continuing after the `std::thread` object is destroyed:

[source,cpp]
----
include::example$snippets/Ca_foundations.cpp[tag=detach,indent=0]
----
...
[source,cpp]
----
include::example$snippets/Ca_foundations.cpp[tag=joinable,indent=0]
----
